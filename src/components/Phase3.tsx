      ...
      <div className="relative w-80 h-64 sm:w-[500px] sm:h-[350px] cursor-pointer transition-all duration-1200 group" onClick={handleOpen}>
        ...
        >
          ...
          <div className="flex flex-col items-center space-y-10 relative z-10">
             <p className="text-[#8B4513]/40 font-medium tracking-[1em] text-[13px] uppercase font-sans">À toi, mon amour</p>
        ...
        {/* FLAP (Top part - Parchment with Torn look) */}
        <motion.div 
        ...
        >
           <div className="w-full h-full flex items-end justify-center pb-8 overflow-visible">
              {!isOpen && (
                /* THE HAND-PRESSED SIGNET WAX SEAL */
                <motion.div 
                  ...
                  style={{ 
                    // Irregular Form for that hand-pressed wax look
                    borderRadius: '45% 55% 52% 48% / 50% 48% 52% 50%',
                    border: '1.5px solid rgba(255,255,255,0.1)'
                  }}
                >
                   {/* Wax "Drip" Shadow details */}
                   ...
                   {/* THE ENGRAVED M - Inner shadowed for depth */}
                   <span 
        ...
        {/* LETTER (Parchment manuscript) */}
        <AnimatePresence>
          {isOpen && (
            <motion.div
              className="absolute inset-0 bg-[#F5F1E6] shadow-3xl mx-4 z-10 p-16 flex flex-col items-center justify-center text-forest deckle-edge rounded-sm border border-white/5"
            ...
            >
              ...
              <p className="font-title italic text-5xl text-[#5D4037] leading-tight px-4 drop-shadow-xs">Ton Instant Précieux</p>
